...
def get_super_admin_dashboard(
    ...
    # 2. Usuários por nível de hierarquia
    ...
    # 3. Usuários logados recentemente (últimas 24 horas)
    ...
    # 4. Usuários ativos/inativos
    active_users = db.query(User).filter(User.is_active == True).count()
    inactive_users = db.query(User).filter(User.is_active == False).count()
    
    # 5. Top 5 organizações por número de usuários
    top_orgs = db.query(
        Organization.name,
    ...
    # 6. Crescimento de organizações (últimos 6 meses)
