...
export default function UsersManagementPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const response = await fetch(`http://localhost:8888/api/v1/users-management/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      if (response.ok) {
        ...
        const error = await response.json();
        alert(error.detail || 'Erro ao desativar usuário');
      }
    } catch (error) {
      console.error('Erro:', error);
                  ...
                  >
                    ...
                    {organizations.map(org => (
                      <option key={org.id} value={org.id}>
                        {org.name} ({statistics.find(s => s.organization_id === org.id)?.current_users || 0}/{org.max_users} usuários)
                      </option>
                    ))}
                  </select>
                </div>
