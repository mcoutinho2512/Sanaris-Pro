...
export default function UsersManagementPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const usersResponse = await fetch('http://localhost:8888/api/v1/users-management/users', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleDelete = async (userId: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:8888/api/v1/users-management/users/${userId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
    ...
    try {
      ...
      const response = await fetch('http://localhost:8888/api/v1/users-management/users', {
        ...
        body: JSON.stringify({
          ...
          role: formData.role,
          password: 'Senha123!'
          // organization_id será preenchido pelo backend automaticamente
        })
      });
